...
class QuadTree<T> {
    ...
    func insert(coordinate: CLLocationCoordinate2D, value: T) -> Bool {
        ...
        if northeast!.insert(coordinate: coordinate, value: value) { return true }
        if northwest!.insert(coordinate: coordinate, value: value) { return true }
        if southeast!.insert(coordinate: coordinate, value: value) { return true }
        if southwest!.insert(coordinate: coordinate, value: value) { return true }
    ...
    private func subdivide() {
        ...
        northeast = QuadTree(bounds: BoundingBox(minLat: midLat, maxLat: bounds.maxLat, minLng: midLng, maxLng: bounds.maxLng), capacity: capacity)
        northwest = QuadTree(bounds: BoundingBox(minLat: midLat, maxLat: bounds.maxLat, minLng: bounds.minLng, maxLng: midLng), capacity: capacity)
        southeast = QuadTree(bounds: BoundingBox(minLat: bounds.minLat, maxLat: midLat, minLng: midLng, maxLng: bounds.maxLng), capacity: capacity)
        southwest = QuadTree(bounds: BoundingBox(minLat: bounds.minLat, maxLat: midLat, minLng: bounds.minLng, maxLng: midLng), capacity: capacity)
